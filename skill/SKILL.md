...
ChatGPT 使用 `GPT-5.6 Sol`，允许强度依次为 `最高` → `极高` → `高` → `中`；DeepSeek 使用 `专家模式`；Qwen 使用上述精确模型和一次回退；Gemini 保持当前已登录会话的模型设置。
