...
When one request needs both bounded long text and approved images, the ChatGPT multimodal route may use `text_transport: 'system_clipboard'` together with `imagePaths`. This combined route keeps the same strict clipboard byte/SHA-256 receipt, one-Provider requirement, `local_fallback: false`, required structured-response validation, and final distinct attachment-count confirmation; it must not broaden the image or text approval scope. If ChatGPT converts a large paste into its pasted-text attachment, use the unique new `在文本字段中显示`/`Show in text field` control to round-trip the content into the composer and confirm its paragraph-reconstructed byte count and SHA-256 before sending; an empty composer alone is not a paste failure.
...
After confirming `聊天`, first inspect the current composer-adjacent strength control. The direct-send policy is explicit: if a visible button or button-role DOM control matches exactly one configured strength (`Pro/最高`、`极高`、`高`、`中`), send directly without opening any selection menu and leave the active ChatGPT model unchanged. The direct branch is marked `modelVerified: false`; it does not claim that the configured model was reselected. Because the new-chat strength control can render after the composer, the initial check waits up to 10 seconds for a configured current tier before opening the full menu. If a rate-limit dialog temporarily strips accessible names, locate the unique visible composer by `#prompt-textarea`/unified-composer structure and the send control by its stable test id after applying the existing dialog recovery; never interpret this as logged out. Recheck after filling the prompt; only if the current strength is missing, ambiguous, or outside the configured tiers must the runner use the composer-adjacent model control → `高级` → `思考强度` and select the first enabled tier in configured order. Do not use `极速`. A menu-selected branch is marked `modelVerified: true`. If this pre-send UI confirmation still fails, do not write a negative Provider health-cache entry; the next request must retry ChatGPT. Runner and Adapter imports are source-mtime versioned so a long-lived JavaScript caller does not silently keep an older implementation after an on-disk update. Only when the selector cannot verify a model or all four tiers are unavailable may the runner continue to DeepSeek `专家模式`. Do not call every provider in parallel.
